...
function checkIfAnyMovieHasRating(movies,[rating = "G"]) {
  if (movies.length === 0) {
    ...
    return movies.some(movie => movie.rating = "G")
   }
...
function getRottenTomatoesScoreByMovie(movies) {
  if (movies.length === 0) {
   ...
   return movies.map(movie => movie.title).find()
    // if 
  // let n = movies.map(movie => movie.title)
  // let k = n.find(ratings.source = "Rotten Tomatoes")
  //  return {n:k})
