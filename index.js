...
function findById(movies,id) {
  ...
  let byId = movies.find((movie) => movie.imdbID === id);
  return byId || null;
}
...
function filterByGenre(movies,genre) {
  ...
  return (byMovieGenre = movies.filter((movie) =>
...
function getAllMoviesReleasedAtOrBeforeYear(movies,year) {
   ...
   return movies.filter((movie) =>
     Number(
       movie.released.slice(
         movie.released.length - 4,
         movie.released.length <= year
       )
     )
   );
}
 ...
 */
function getRottenTomatoesScoreByMovie(movies) {if (!movies.length) {
  throw 'movies array is empty';
}
return movies.map((movie) => {
  return {
